...
class TrackDatabase:
            ...
            for listtype in ["include", "exclude"]:
                if listtype in playlist:
                    if isinstance(playlist[listtype], list):
                        ...
                        self.playlists[playlist["name"]][listtype] = [
                            playlist[listtype]
                        ]
        ...
        for entry in self.group_config[style]:
            if "folder" not in entry or entry["folder"] in relpath:
                ...
                for regex in entry["regex"]:
                    ...
                    if matches:
                        try:
                            self.add_to_grouping(
                                relpath,
                                style,
                                f"{entry['name']}_{folder}_{matches.group(1).strip()}",
                            )
                        except IndexError:
                            self.add_to_grouping(
                                relpath, style, f"{entry['name']}_{folder}"
                            )
                        return
    ...
    def gather_database(self):
        ...
        for root, _, files in os.walk(self.basepath):
            ...
            for fn in filter(TrackDatabase.MUSIC_FILES.search, sorted(files)):
                filename = os.path.join(root, fn).replace("\\", "/")
                playpath = os.path.join(relpath, fn).replace("\\", "/")
    ...
    def get_playset_includes(self, *, inclusions, style):
        if len(inclusions) == 0:
            try:
                ...
                raise KeyError(f"Could not find {style} in {self.groupings}")
        songset = {}
    ...
    def build_playsets(self):
        for playlist in self.playlists:
            ...
            self.remove_playset_excludes(
                playset=self.playlists[playlist]["grouplist"],
    ...
    def make_playlists(self, folder):
        ...
        for playlist in self.playlists:
            ...
            for group in groups:
                self.playlists[playlist]["tracklist"].extend(
                    self.playlists[playlist]["grouplist"][group]
                )
            abslist = Playlist(
                [
                    f"{self.basepath.rstrip('/')}/{path}"
                    for path in self.playlists[playlist]["tracklist"]
                ]
            )
            abslist.write_file(
                f"{folder}/relative/{playlist}.pls", relpath=self.basepath
            )
